...
class _CustomDropdownMenuState<T> extends State<CustomDropdownMenu<T>> {
  ...
  void initState() {
    ...
    _menuHasEnabledItem = filteredEntries.any((CustomDropdownMenuEntry<T> entry) => entry.enabled);

    final index = filteredEntries.indexWhere((CustomDropdownMenuEntry<T> entry) => entry.value == widget.initialSelection);
    if (index != -1) {
  ...
  List<CustomDropdownMenuEntry<T>> filter(List<CustomDropdownMenuEntry<T>> entries, TextEditingController textEditingController) {
    final filterText = textEditingController.text.toLowerCase();
    return entries.where((CustomDropdownMenuEntry<T> entry) => entry.label.toLowerCase().contains(filterText)).toList();
  }

  int? search(List<CustomDropdownMenuEntry<T>> entries, TextEditingController textEditingController) {
    final searchText = textEditingController.value.text.toLowerCase();
    if (searchText.isEmpty) {
      return null;
    }
    final index = entries.indexWhere((CustomDropdownMenuEntry<T> entry) => entry.label.toLowerCase().contains(searchText));

    return index != -1 ? index : null;
  }

  List<Widget> _buildButtons(List<CustomDropdownMenuEntry<T>> filteredEntries, TextEditingController textEditingController, TextDirection textDirection, {int? focusedIndex, bool enableScrollToHighlight = true}) {
    final result = <Widget>[];
    for (var i = 0; i < filteredEntries.length; i++) {
      ...
      // the text field.
      final padding = entry.leadingIcon == null ? (leadingPadding ?? _kDefaultHorizontalPadding) : _kDefaultHorizontalPadding;
      ...
      final focusedBackgroundColor = effectiveStyle.foregroundColor?.resolve(<MaterialState>{MaterialState.focused}) ?? Theme.of(context).colorScheme.onSurface;
      ...
      effectiveStyle = entry.enabled && i == focusedIndex ? effectiveStyle.copyWith(backgroundColor: MaterialStatePropertyAll<Color>(focusedBackgroundColor.withOpacity(0.12))) : effectiveStyle;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.width != null) {
      effectiveMenuStyle = effectiveMenuStyle.copyWith(minimumSize: MaterialStatePropertyAll<Size?>(Size(widget.width!, 0)));
    } else if (anchorWidth != null) {
      effectiveMenuStyle = effectiveMenuStyle.copyWith(minimumSize: MaterialStatePropertyAll<Size?>(Size(anchorWidth, 0)));
    }
    ...
    Widget menuAnchor = MenuAnchor(
      ...
      builder: (BuildContext context, MenuController controller, Widget? child) {
        assert(_initialMenu != null, 'error');
        final Widget trailingButton = IconButton(
          isSelected: controller.isOpen,
          icon: widget.trailingIcon ?? Assets.images.icDropDown.svg(color: Theme.of(context).iconTheme.color),
          selectedIcon: widget.selectedTrailingIcon ??
              Transform.rotate(
                angle: -3.14,
                child: Assets.images.icDropDown.svg(color: Theme.of(context).iconTheme.color),
              ),
          onPressed: () {
        ...
        final Widget leadingButton = Padding(padding: const EdgeInsets.all(8), child: widget.leadingIcon ?? const SizedBox());
    ...
    if (widget.expandedInsets != null) {
      menuAnchor = Container(
        ...
        padding: widget.expandedInsets?.copyWith(top: 0, bottom: 0),
        child: menuAnchor,
      );
    }
  ...
  void performLayout() {
    ...
    while (child != null) {
      if (child == firstChild) {
        ...
        assert(child.parentData == childParentData, 'error');
    ...
    if (child != null) {
      ...
      final isHit = result.addWithPaintOffset(
        ...
        hitTest: (BoxHitTestResult result, Offset transformed) {
          assert(transformed == position - childParentData.offset, 'error');
...
class _DropdownMenuDefaultsM3 extends DropdownMenuThemeData {
  ...
  MenuStyle get menuStyle {
    return const MenuStyle(
      minimumSize: MaterialStatePropertyAll<Size>(Size(_kMinimumWidth, 0)),
